/*
 * Copyright (c) 2022, Reinier Hasper, sendrato.com
...
//#define LOG_DBG_ENABLED IS_ENABLED(CONFIG_BT_DEBUG_HCI_DRIVER)
                                    ...
                                    void *packet, uint16_t size )
{
	...
	switch (packet_type) {
        case HCI_EVT:
            ...
            evt_data = (struct bt_hci_evt_le_meta_event *) &(((const uint8_t *)packet)[BT_HCI_EVT_HDR_SIZE]);
            ...
            if (!buf) {
                if (discardable) {
                    LOG_DBG("Discard EVT due to insufficient buf, ignoring event [to:%llx]", timeout.ticks);
                } else {
                    LOG_ERR("Discard EVT due to insufficient buf [to:%llx]", timeout.ticks);
                }
