...
#include <zephyr/drivers/nfc/nrfx/nfc_t2t_lib.h>
#endif
#if CONFIG_NFC_T4T_NRFXLIB
#include <zephyr/drivers/nfc/nrfx/nfc_t4t_lib.h>
#endif
...
static uint8_t _nrfxnfc_payload[CONFIG_NRFXNFC_MAX_PAYLOAD_SIZE];

/******************* NFC SUBSYS DRIVER IMPLEMENTATION *******************/


#if CONFIG_NFC_T2T_NRFXLIB
static void nrfxnfc_t2t_cb(void *context,
                                ...
                                uint8_t *buf, uint16_t len)
{
    ...
    if (len > CONFIG_NRFXNFC_MAX_PAYLOAD_SIZE) {
        return -ENOMEM;
    }

    /* copy buf to ensure longevity and validity of data */
    (void) memset(_nrfxnfc_payload, 0, CONFIG_NRFXNFC_MAX_PAYLOAD_SIZE);
...
static char _nrfxnfc_driver_name[] = CONFIG_NRFXNFC_DRV_NAME;
