/*
 * Copyright (c) 2021, Sendrato.com, All rights reserved.
 *
 * This software is the confidential and proprietary information of Sendrato
 * and is subject to the terms and conditions defined in file 'LICENSE.txt',
 * which is part of this source code package. You shall not disclose such
 * Confidential Information and shall use it only in accordance with the terms
 * of the license agreement you entered into with Sendrato.
 *
 * 2021, Sendrato, Hessel van der Molen
 */
...
#define NT3H2X11_CSREG_SET_EN(_d, _csreg, _r, _m, _v) \
        _nt3h2x11_write_csreg_register_enable(_d, _csreg, \
                                                  NT3H2X11_REG_##_r, \
                                                  NT3H2X11_MSK_##_r##_##_m, \
                                                  _v)
...
#define NT3H2X11_CSREG_GET_EN(_d, _csreg, _r, _m, _v) \
        _nt3h2x11_read_csreg_register_enable(_d, _csreg, \
                                                 NT3H2X11_REG_##_r, \
                                                 NT3H2X11_MSK_##_r##_##_m, \
                                                 _v)
...
#define NT3H2X11_CSREG_SET_VAL(_d, _csreg, _r, _m, _v) \
        _nt3h2x11_write_csreg_register_value(_d, _csreg, \
                                                 NT3H2X11_REG_##_r, \
                                                 NT3H2X11_MSK_##_r##_##_m, \
                                                 _v)
...
#define NT3H2X11_CSREG_GET_VAL(_d, _csreg, _r, _m, _v) \
        _nt3h2x11_read_csreg_register_value(_d, _csreg, \
                                                NT3H2X11_REG_##_r, \
                                                NT3H2X11_MSK_##_r##_##_m, \
                                                _v)
 ...
 */
int _nt3h2x11_read_csreg_register(const struct device *dev,
                                  enum nt3h2x11_csreg csreg,
                                  uint8_t reg_addr,
                                  uint8_t *val)
{
 ...
 * @return              : 0 on success, negative upon error.
 */
static int _nt3h2x11_write_csreg_register(const struct device *dev,
                                          enum nt3h2x11_csreg csreg,
                                          uint8_t reg_addr,
 ...
 * @return               : 0 on success, negative upon error.
 */
static int _nt3h2x11_read_csreg_register_enable(const struct device *dev,
                                                enum nt3h2x11_csreg csreg,
                                                uint8_t reg,
                                                uint8_t mask,
                                                uint8_t *val)
 ...
 * @return              : 0 on success, negative upon error.
 */
static int _nt3h2x11_write_csreg_register_value(const struct device *dev,
                                                enum nt3h2x11_csreg csreg,
                                                uint8_t reg,
                                                uint8_t mask,
                                                uint8_t val)
 ...
 * @return               : 0 on success, negative upon error.
 */
static int _nt3h2x11_read_csreg_register_value(const struct device *dev,
                                               enum nt3h2x11_csreg csreg,
                                               uint8_t reg,
                                               uint8_t mask,
                                               uint8_t *val)
...
#if 0
// TODO: check if SESSION and CONFIG can be partially written/read or
//       need block read/write

static int _nt3h2x11_read_config_register(const struct device *dev,
                                          uint8_t reg_addr,
                                          uint8_t *val)
{
    uint8_t rbuf[NT3H2X11_BYTES_BLK];
    int rv = _nt3h2x11_read_block(dev, NT3H2X11_BLK_CONFIG,
                                  rbuf, NT3H2X11_BYTES_BLK);
    if (rv != 0) {
        return rv;
    }
    /* fetch register from config-block */
    *val = rbuf[reg_addr];
    return 0;
}

static int _nt3h2x11_write_config_register(const struct device *dev,
                                           uint8_t reg_addr,
                                           uint8_t val_mask,
                                           uint8_t val)
{
    ...
    /* clear old val-bits */
    rbuf[reg_addr] &= (uint8_t) ~val_mask;
    /* update val to config-block */
    rbuf[reg_addr] |= (uint8_t) (val & val_mask);
    /* update block */
    return _nt3h2x11_write_block(dev, NT3H2X11_BLK_CONFIG,
                                 rbuf, NT3H2X11_BYTES_BLK);
}
#endif
...
int nt3h2x11_set_cfg_auth(const struct device *dev,
                          struct nt3h2x11_cfg_auth *cfg_auth)
{
    int rv = 0;
    ...
    (void) memcpy(&blk_la[NT3H2X11_REG_DYNAMIC_LOCK],
                  cfg_auth->dyn_lock, NT3H2X11_BYTES_DYNAMIC_LOCK);
    (void) memcpy(&blk_la[NT3H2X11_REG_AUTH0],
                  cfg_auth->auth0, NT3H2X11_BYTES_AUTH0);
    (void) memcpy(&blk_ap[NT3H2X11_REG_ACCESS],
                  cfg_auth->access, NT3H2X11_BYTES_ACCESS);
    (void) memcpy(&blk_ap[NT3H2X11_REG_PWD],
                  cfg_auth->pwd, NT3H2X11_BYTES_PWD);
    (void) memcpy(&blk_ap[NT3H2X11_REG_PACK],
                  cfg_auth->pack, NT3H2X11_BYTES_PACK);
    (void) memcpy(&blk_ap[NT3H2X11_REG_PT_I2C],
                  cfg_auth->pt_i2c, NT3H2X11_BYTES_PT_I2C);
...
int nt3h2x11_get_cfg_auth(const struct device *dev,
                          struct nt3h2x11_cfg_auth *cfg_auth)
{
    int rv = 0;
    uint8_t blk_la[NT3H2X11_BYTES_BLK]; /* lock&auth */
    ...
    (void) memcpy(cfg_auth->dyn_lock,
                  &blk_la[NT3H2X11_REG_DYNAMIC_LOCK], NT3H2X11_BYTES_DYNAMIC_LOCK);
    (void) memcpy(cfg_auth->auth0,
                  &blk_la[NT3H2X11_REG_AUTH0], NT3H2X11_BYTES_AUTH0);
    (void) memcpy(cfg_auth->access,
                  &blk_ap[NT3H2X11_REG_ACCESS], NT3H2X11_BYTES_ACCESS);
    (void) memcpy(cfg_auth->pwd,
                  &blk_ap[NT3H2X11_REG_PWD], NT3H2X11_BYTES_PWD);
    (void) memcpy(cfg_auth->pack,
                  &blk_ap[NT3H2X11_REG_PACK], NT3H2X11_BYTES_PACK);
    (void) memcpy(cfg_auth->pt_i2c,
                  &blk_ap[NT3H2X11_REG_PT_I2C], NT3H2X11_BYTES_PT_I2C);
...
int nt3h2x11_set_softreset_en(const struct device *dev,
                              enum nt3h2x11_csreg csreg,
                              uint8_t enable)
{
...
int nt3h2x11_get_softreset_en(const struct device *dev,
                              enum nt3h2x11_csreg csreg,
                              uint8_t *enable)
{
...
int nt3h2x11_set_pthru_en(const struct device *dev,
                          enum nt3h2x11_csreg csreg,
                          uint8_t enable)
{
...
int nt3h2x11_get_pthru_en(const struct device *dev,
                          enum nt3h2x11_csreg csreg,
                          uint8_t *enable)
{
...
int nt3h2x11_set_fd_off(const struct device *dev,
                        enum nt3h2x11_csreg csreg,
...
int nt3h2x11_get_fd_off(const struct device *dev,
                        enum nt3h2x11_csreg csreg,
                        enum nt3h2x11_fd_off *fd_off)
{
...
int nt3h2x11_set_fd_on(const struct device *dev,
                       enum nt3h2x11_csreg csreg,
...
int nt3h2x11_get_fd_on(const struct device *dev,
                       enum nt3h2x11_csreg csreg,
                       enum nt3h2x11_fd_on *fd_on)
{
...
int nt3h2x11_set_srammirror_en(const struct device *dev,
                               enum nt3h2x11_csreg csreg,
                               uint8_t enable)
{
...
int nt3h2x11_get_srammirror_en(const struct device *dev,
                               enum nt3h2x11_csreg csreg,
                               uint8_t *enable)
{
...
int nt3h2x11_set_transfer_dir(const struct device *dev,
                              enum nt3h2x11_csreg csreg,
...
int nt3h2x11_get_transfer_dir(const struct device *dev,
                              enum nt3h2x11_csreg csreg,
                              enum nt3h2x11_transfer_dir *dir)
{
...
int nt3h2x11_set_last_ndef_blk(const struct device *dev,
                               enum nt3h2x11_csreg csreg,
                               uint8_t block)
{
    ...
    return _nt3h2x11_write_csreg_register(dev, csreg,
                                          NT3H2X11_REG_LAST_NDEF_BLOCK,
                                          0xFFu, block);
}

int nt3h2x11_get_last_ndef_blk(const struct device *dev,
                               enum nt3h2x11_csreg csreg,
                               uint8_t *block)
{
    return _nt3h2x11_read_csreg_register(dev, csreg,
                                         NT3H2X11_REG_LAST_NDEF_BLOCK,
                                         block);
}

int nt3h2x11_set_srammirror_blk(const struct device *dev,
                                enum nt3h2x11_csreg csreg,
                                uint8_t block)
{
    return _nt3h2x11_write_csreg_register(dev, csreg,
                                          NT3H2X11_REG_SRAM_MIRROR_BLOCK,
                                          0xFFu, block);
}

int nt3h2x11_get_srammirror_blk(const struct device *dev,
                                enum nt3h2x11_csreg csreg,
                                uint8_t *block)
{
    return _nt3h2x11_read_csreg_register(dev, csreg,
...
int nt3h2x11_set_wdt(const struct device *dev,
                     enum nt3h2x11_csreg csreg,
                     uint16_t time)
{
    int rv = 0;
    ...
    /* Write LSB */
    val = (time >> 0u) & 0xFFu;
    rv = _nt3h2x11_write_csreg_register(dev, csreg,
                                        NT3H2X11_REG_WDT_LS, 0xFFu, val);
    if (rv != 0) {
        return rv;
    }
...
int nt3h2x11_get_wdt(const struct device *dev,
                     enum nt3h2x11_csreg csreg,
                     uint16_t *time)
{
    int rv = 0;
    ...
    *time |= (((uint16_t) regval) << 0u) & 0x00FFu;
    ...
    *time |= (((uint16_t) regval) << 8u) & 0xFF00u;

    return 0;
}

int nt3h2x11_set_i2c_clkstr_en(const struct device *dev,
                               enum nt3h2x11_csreg csreg,
                               uint8_t enable)
{
...
int nt3h2x11_get_i2c_clkstr_en(const struct device *dev,
                               enum nt3h2x11_csreg csreg,
                               uint8_t *enable)
{
    return NT3H2X11_CSREG_GET_EN(dev, csreg, I2C_CLOCK_STR,
                                 CLOCK_STR, (uint8_t *) enable);
}
...
{
    return NT3H2X11_CSREG_GET_EN(dev, NT3H2X11_CONFIG,
                                 CFG_REG_LOCK, LOCK_I2C, locked);
}

int nt3h2x11_get_nfc_lock_config(const struct device *dev, uint8_t *locked)
{
    return NT3H2X11_CSREG_GET_EN(dev, NT3H2X11_CONFIG,
                                 CFG_REG_LOCK, LOCK_NFC, locked);
}
...
{
    return NT3H2X11_CSREG_GET_EN(dev, NT3H2X11_SESSION,
                                 I2C_CLOCK_STR, NEG_AUTH, enable);
}


int nt3h2x11_get_nsreg(const struct device *dev, uint8_t *nsreg)
{
    return _nt3h2x11_read_csreg_register(dev, NT3H2X11_SESSION,
...
int nt3h2x11_read_blocks(const struct device *dev, uint8_t block,
                         uint8_t *buf, uint8_t count)
{
    ...
    for (uint8_t i = 0u; i < count; i++) {
        ...
        rv = i2c_write_read(data->dev_i2c, cfg->i2c.addr,
                            &block_idx, sizeof(block_idx),
...
int nt3h2x11_write_blocks(const struct device *dev, uint8_t block,
                          uint8_t *buf, uint8_t count)
{
    int rv = 0;
    uint32_t timeout = 0u;
    ...
    for (uint8_t i = 0u; i < count; i++) {
        ...
        if ((block <= NT3H2X11_BLK_SRAM_START) ||
            (block > (NT3H2X11_BLK_SRAM_START + (NT3H2X11_BYTES_SRAM / NT3H2X11_BYTES_BLK)))) {

            /* number of timeouts to wait */
            timeout = (CONFIG_NT3H2X11_MAX_WRITE_DELAY / 5u) + 1u;
            do {
                ...
                timeout--;
            } while ((timeout != 0u) && ((nsreg & NT3H2X11_MSK_NS_REG_EEPROM_WR_BUSY) != 0u));
...
int nt3h2x11_read_bytes(const struct device *dev, uint16_t addr,
...
int nt3h2x11_write_bytes(const struct device *dev, uint16_t addr,
                         uint8_t *buf, uint16_t buf_len)
{
    int rv;
...
int nt3h2x11_set_i2c_addr(const struct device *dev,
                          uint8_t addr_old,
                          uint8_t addr_new)
{
    int rv = 0;
    ...
    rv = i2c_write_read(data->dev_i2c, addr_old,
                        &block_idx, sizeof(block_idx),
                        &buf[1], NT3H2X11_BYTES_BLK);
    if (rv != 0) {
        LOG_DBG("Can't update I2C (%02x => %02x). Read Error: %d",
                addr_old, addr_new, rv);
        return rv;
    }
    ...
    if (rv != 0) {
        LOG_DBG("Can't change I2C (%02x => %02x). Write Error: %d",
                addr_old, addr_new, rv);
        return rv;
    }
...
static enum nt3h2x11_event _nt3h2x11_reg2event(const struct device *dev,
        ...
        case NT3H2X11_FD_OFF_RF_OFF_OR_LAST_DATA_RW:
            ...
            /* only when pass-through is enabled */
                ...
                if (fd_on == NT3H2X11_FD_ON_RF_ON) {
                    /* .. if last data is read by I2C (pthru:NFC>I2C) */
                    if (dir == NT3H2X11_TRANSFER_DIR_RF_TO_I2C) {
                        if (flag_i2c_locked == 0u) {
                            event = NT3H2X11_EVENT_LAST_DATA_READ_I2C;
                        }
                    /* .. or last data is written by I2C (pthru:I2C>NFC) */
                    } else {
                        if (flag_nfc_sram_ready != 0u) {
...
int nt3h2x11_irq_set_callback(const struct device *dev,
                              nt3h2x11_irq_callback_t cb)
{
    struct nt3h2x11_data *data = (struct nt3h2x11_data *) dev->data;
    data->app_irq_cb = cb;
    return 0;
...
static void _nt3h2x11_irq_cb_worker(struct k_work *worker)
{
    ...
    (void) _nt3h2x11_read_csreg_register(dev, NT3H2X11_SESSION,
                                         NT3H2X11_REG_NC_REG, &nc_reg);
    (void) _nt3h2x11_read_csreg_register(dev, NT3H2X11_SESSION,
                                         NT3H2X11_REG_NS_REG, &ns_reg);
...
static void _nt3h2x11_irq_external_cb(const struct device *dev,
                                      struct gpio_callback *cb,
                                      uint32_t pins)
{
    struct nt3h2x11_data *data = CONTAINER_OF(cb, struct nt3h2x11_data,
                                              dev_irq_external_cb);
...
static void nt3h2x11_tag_irq_cb(const struct device *dev,
                                enum nt3h2x11_event event,
                                uint8_t nc_reg, uint8_t ns_reg)
{
    ARG_UNUSED(nc_reg);
    ARG_UNUSED(ns_reg);
...
static int nt3h2x11_tag_get_type(const struct device *dev,
...
static int nt3h2x11_tag_set_ndef(const struct device *dev,
...
static int nt3h2x11_tag_cmd(const struct device *dev,
...
static int _nt3h2x11_init_external(const struct device *dev)
{
    ...
    if (data->dev_irq_external != NULL) {
        ...
        gpio_init_callback(&data->dev_irq_external_cb,
                           _nt3h2x11_irq_external_cb, BIT(cfg->irq_gpio.pin));

        rv = gpio_add_callback(data->dev_irq_external,
                               &data->dev_irq_external_cb);
        if (rv != 0) {
