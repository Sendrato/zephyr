...
static bool load_calibration_param_from_flash(ADC_Type *base)
{
    ...
    do
    {
        ...
        if (!is_valid)
        {
            printk("Calibration data loading error: no ADC calibration data available!\r\n");
            break;
        }
        ...
        if (!is_valid)
        {
            printk("Calibration data loading error: no temperature sensor calibration data available!\r\n");
            break;
        }
...
static void get_temperature(ADC_Type *base, uint8_t channel, uint8_t nb_samples, int32_t *temperature)
{
    ...
    /**********************************************/
#ifdef DBG_DRIVER
    printk("ADC output code from current temperature\r\n");
#endif
...
#ifdef DBG_DRIVER
    printk("tsens_adcout_T_sum8 = %d\r\n", tsens_adcout_T_sum8);
    printk("tsens_adcout_T = %d\r\n", tsens_adcout_T_sum8 / nb_samples);
    printk("\r\n");
#endif
    ...
    /*************************************/
#ifdef DBG_DRIVER
    printk("ADC output code from current VBAT\r\n");
#endif
...
int32_t BOARD_GetTemperature(void)
{
    ...
    else
    {
        ...
        printk("FAIL: temperature measurement failed due to wrong ADC configuration or missing calibration data!\r\n");
    }
#endif
